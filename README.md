...
- Домен "Управление пользователями и домами":
	- Функции: создание, редактирование и удаление пользователей и домов
	- Сущности: Пользователь, Дом
	- Связанные данные: Данные пользователей и домов

- Домен "Управление устройствами и сценариями":
	- Функции: регистрация устройств, статус онлайн/офлайн, хранение и исполнение сценариев
	- Сущности: Устройство, Сценарий
	- Связанные данные: Данные устройств и сценариев

- Домен "Управление телеметрией":
	- Функции: принимает и хранит потоки данных с устройств, предоставляет API для истории и текущих значений
	- Сущности: Телеметрия
	- Связанные данные: Данные телеметрии

- Домен "Управление каталогами и платежами":
	- Функции: управляет каталогами устройств от партнёров, обрабатывает покупку модулей, интеграция с платёжной системой.
	- Сущности: Заказ, Карточки товаров (Модулей), Транзакция
	- Связанные данные: Данные заказа, устройств партнёров и платёжных систем
...
1. Переходим в apps
2. Пишем docker compose up
3. Загружаем новую коллекцию в постман [Новая коллекция Postman](https://github.com/AndKiRiLL/architecture-pro-warmhouse/blob/warmhouse/apps/NEW-smart-home-device-api.postman_collection.json)
5. В постман есть полный CRUD для device, работает с таблицей device не sensor. При старте создаётся тестовый один device
6. Есть endpoint Get Telemetry by device ID, возвращает нам всю телеметрию (Микросервис сам создаёт для каждого устройства раз в три секунды телеметрию)